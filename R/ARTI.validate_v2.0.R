ARTI.validate_v2.0<-function(obs.csv,radar.designation,weather.station.code,local.time.zone){
  #Elwood: It's 106 miles to Chicago, we got a full tank of gas, half a pack of cigarettes, it's dark... and we're wearing sunglasses.
  #Jake: Hit it.
  
  #SLC Validate v_0.1
  ##Everybody good? Plenty of slaves for my robot colony?
  ##SLC Validate v_1.0 Bugs squashed
  ##Look. Just give me some inner peace or I'll mop the floor with ya!
  ##SLC Validate v_1.5 Some warnings addressed
  ##Men have become the tools of their tools. 
  ##v2.0 - Modified for SEA, added full tracks to output, parameters file output, map extents (open street maps?)... weather? Multiple radars simultaneously?
  ...
  #################### <MODIFY ME> #####################  
    #Time Allowance: +/- this value
      time.allowance <- 5 ##(seconds)
      assign("time.allowance",time.allowance,.GlobalEnv)
    #Bearing Allowance: +/- this value
      bearing.allowance <- 10 ##(degrees)
    #Inclination Allowancd: ##degrees - Only cosmetic
      inclination.allowance <-4
      assign("bearing.allowance",bearing.allowance,.GlobalEnv)
    #Search Distance from observer (m)
      search.distance <- 8046
      assign("search.distance",search.distance,.GlobalEnv)
    #Overhead radius
      overhead.radius <- 1500
      assign("overhead.radius",overhead.radius,.GlobalEnv)
  ...
  #Make R understand observation data
    obs$BEARING..deg.<-suppressWarnings(as.numeric(levels(obs$BEARING..deg.))[obs$BEARING..deg.]) #need to preserve NA values
    obs$INCL...deg.<-suppressWarnings(as.numeric(levels(obs$INCL...deg.))[obs$INCL...deg.]) #need to preserve NA values
    obs$BEARING..deg.<-circular(obs$BEARING..deg.,type="angles",units="degrees",template="none",modulo="2pi",rotation="clock") ##Bearings are circular data
    obs$HEADING.N..NNW.<-as.character(obs$HEADING.N..NNW.)
    obs$NOTES<-as.character(obs$NOTES) ##Notes are characters, not factors
    obs$Date.Time.UTC<- as.POSIXct(obs$Date.Time.UTC,format="%m/%d/%Y %H:%M:%S",tz="UTC") ##Time as time object with correct time zone
    obs$Date.Time.local<-as.POSIXct(format(obs$Date.Time.UTC,tz = local.time.zone),tz = local.time.zone)
    utmcoor<- SpatialPoints(cbind(obs$Easting,obs$Northing), proj4string=CRS("+proj=utm +zone=10T")) ##Classify as Spatial Data. Mind the UTM Zone
    ...
    obs$breaks<-cut(obs$Date.Time.UTC,breaks="day")
    assign("obs",obs,.GlobalEnv)
    obs$round<-as.POSIXct(round(as.numeric(obs$Date.Time.local)/(30*60))*(30*60),origin=(as.POSIXlt('1970-01-01'))) ##will need this to match with weather data from KSEA, rounds timestamp to nearest half hour to match metar interval
  
  #Where are you?    
    site.name<-revgeocode(c(obs$obs.Longitude[1],obs$obs.Latitude[1]),output = "more") 
  
  #Any interesting weather where you is?
    start.date<-as.Date(format(min(obs$Date.Time.UTC),tz = local.time.zone))
    end.date<-as.Date(format(max(obs$Date.Time.UTC),tz = local.time.zone))
  ...
  data$Update.Time<-as.POSIXct(data$Update.Time,format="%Y/%m/%d %H:%M:%S",tz="UTC") ##Time as time object with correct time zone
  ...
  for(i in 1:nrow(obs)){
    ...
    if(obs$INCL...deg.[i] != 90 | is.na(obs$INCL...deg.[i])==TRUE){
      #Calculate range of acceptable bearings
      bearing.low <- ifelse(obs$BEARING..deg.[i] - bearing.allowance/2 < 0, 360 + (obs$BEARING..deg.[i] - bearing.allowance/2), obs$BEARING..deg.[i] - bearing.allowance/2)
      bearing.high <- ifelse(obs$BEARING..deg.[i] + bearing.allowance/2 > 360, (obs$BEARING..deg. + bearing.allowance/2) - 360, obs$BEARING..deg.[i] + bearing.allowance/2)
      #Calculate bearing to track update from observer
      sub.data$bearing.to.target <- bearing(cbind(obs$obs.Longitude[i],obs$obs.Latitude[i]),cbind(sub.data$Longitude,sub.data$Latitude)) ##Calculate bearing to track update
      ...
      sub.time <- subset(sub.data, Update.Time >= obs$Date.Time.UTC[i] - time.allowance & Update.Time <= obs$Date.Time.UTC[i] + time.allowance)
      ...
      if(nrow(validate.list) > 0){
        validate.list$Obs.Time<-obs$Date.Time.UTC[i] #Put the observation time into the validated set
        ...
        filename.time<-as.character(obs$Date.Time.UTC[i],format="%Y.%m.%d_%H%M%S")
        graph.title.time<-as.character(obs$Date.Time.UTC[i],format="%Y-%m-%d %H:%M:%S")
        csv.filename<-paste0("R_",filename.time,"_",bearing.allowance,"_FOV_",obs$BEARING..deg.[i],"_bearing_",obs$HEADING.N..NNW.[i],"_heading",".csv")
        ...
        colnames(fov.low)<-c("Longitude","Latitude","Description")
        ...
        colnames(fov.high)<-c("Longitude","Latitude","Description")
        ...
        colnames(observer)<-c("Longitude","Latitude","Description")
        #Radar Location
        colnames(radar.location)<-c("Longitude","Latitude","Description")
        ...
        full.tracks$Description<-paste("Track",full.tracks$Track.ID)
        ##Create a Dataframe containing all of the longitude/latitude info
        bounds<-rbind.fill(fov.low,fov.high,observer,radar.location,validate.list,full.tracks)
        assign("bounds",bounds,.GlobalEnv)
        
        ##Download a map that fits the data
        map<-get_map(location = make_bbox(lon=Longitude,lat=Latitude,data=bounds,f=1), source="google",maptype = map.type)
        ...
        graph.title<-paste0("Salt Lake City International Airport\n","Observation Time: ",as.character(obs$Date.Time.UTC[i])," UTC\n","Record Number: ",obs$Record.Number[i],"\n","Time Allowance: +/- ",time.allowance," sec\n","FOV: ",bearing.allowance," deg \n","Observation Bearing: ",obs$BEARING..deg.[i],"deg \n","Inclination: ",obs$INCL...deg.[i]," deg\n", obs$X.[i]," ",as.character(obs$SPECIES..code.[i])," heading ",obs$HEADING.N..NNW.[i],"\n","Flight Characteristics: ",as.character(obs$BEHAVIOR..Code.[i]),", ",as.character(obs$NEAR.FAR[i]),"\n", "Notes: ",obs$NOTES[i])
        ...
        q<- p + geom_point(data=subset(bounds,Description != "FOV Line"),aes(x=Longitude,y=Latitude,color=Description,shape=Description),size=2.5,alpha=0.7) + scale_shape(solid=F) + scale_colour_hue(l=40)
        ...
        w<- v + theme(legend.justification = c(.95,.05),legend.position = c(.95,.05),legend.background = element_rect(fill=alpha('grey', 0.6)),legend.text = element_text(size=6),legend.key = element_rect(colour = "black"),legend.title = element_text(face = "italic",size=8),axis.text.x=element_blank(),axis.text.y=element_blank(),axis.line=element_blank(),axis.text.x=element_blank(),axis.text.y=element_blank(),axis.ticks=element_blank(),axis.title.x=element_blank(),axis.title.y=element_blank())       
        #Cow Plot?
        fig1<-plot_grid(track.altitude.4,track.speed.5,track.rcs.5,nrow=3,align="v")
        fig2<-plot_grid(w,fig1,ncol=2,align="h")
        #Filename for Map
        #What to call it?  
        map.filename<-paste0("R_",filename.time,"_",bearing.allowance,"_FOV_",obs$BEARING..deg.[i],"_bearing_",obs$HEADING.N..NNW.[i],"_heading",".jpeg")
        #Save it  
        ggsave(filename = file.path(working.directory,"Results","Map",map.filename),plot = fig2, width= 12,height=6,pointsize=12,units="in" )
      }
      ...
      if(nrow(validate.list) > 0){
        ...
        circ.utmcoor<- SpatialPoints(cbind(xx,yy), proj4string=CRS("+proj=utm +zone=12T")) ##Classify as Spatial Data. Mind the UTM Zone
        ...
        p<- ggmap(map,extent = "normal",maprange = F)
        q<- p + geom_point(data=subset(bounds,Description != "Overhead Radius"),aes(x=Longitude,y=Latitude,color=Description,shape=Description),size=2.5,alpha=0.7) + scale_shape(solid=F) + scale_colour_hue(l=40)
