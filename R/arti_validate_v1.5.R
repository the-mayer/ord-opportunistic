validate <- function(start.time, end.time, fov, target.heading, obs.bearing, obs.UTM.Northing, obs.UTM.Easting, obs.dist.estimate.yards, obs.alt.estimate.ft, weather, radar, csv,oh.factor,USDA.grid.loc,time.allowance,dist.allowance.factor,USDA.number.of.birds,USDA.species,USDA.flight.behavior,USDA.accuracy,USDA.comments) 
  ...
  {#Load Required Packages and Data
    ...
    #Create an output directory
      ...
      dir.create(file.path(working.directory, subDir, subDirCSV), showWarnings = FALSE)
      dir.create(file.path(working.directory, subDir, subDirPNG), showWarnings = FALSE)
      ...
      csv.oh.filename <- paste0("R_",radar,"_",filename.time,"_",obs.UTM.Northing,"N_",obs.UTM.Easting,"E_",fov,"FOV_",obs.bearing,"bearing_",target.heading,"_heading_","OH.csv")
    ...
    #Observer Bird Position
      ...
      beam.width.at.bird.guess<-sqrt((radar.Easting-obs.bird.x)^2 + (radar.Northing-obs.bird.y)^2)*tan(1*pi/180)
...
#Start finding tracks      
      #Subset data based on observer time.interval. Allow 10 seconds on either side to account for radar processing/time sync issues
        sub.time <- subset(data, UNIX.Update.Time >= start.time - time.allowance & UNIX.Update.Time <= end.time + time.allowance)
      ...
      #Calculate difference between observer altitude estimate and top and bottom of the beam
        sub.time$alt.diff.to.top<-ifelse((obs.alt.estimate.meters > sub.time$high.beam.height.m),sub.time$high.beam.height.m - obs.alt.estimate.meters,NA)
        sub.time$alt.diff.to.bottom<-ifelse((obs.alt.estimate.meters < sub.time$low.beam.height.m), obs.alt.estimate.meters - sub.time$low.beam.height.m,NA)
      ...
      #Determine if Observation is Overhead based on obs.bearing variable and do something different if it is
        if(obs.dist.estimate.meters == 0 | obs.dist.estimate.meters <= beam.width.at.bird.guess ) {
          sub.bearing <- subset(sub.heading, distance.to.target.m <= oh.factor * obs.alt.estimate.meters + beam.width.at.bird.guess) ##Radius (m) = oh.factor * beam width at estimated bird location
        }
        else{
      ...
      #Subset based on low and high bearings and store as new dataframe (sub.bearing)
        if(high.bearing.1 > low.bearing.1){
          ...
          sub.bearing <-subset(sub.bearing,distance.to.target.m <= dist.allowance.factor * obs.dist.estimate.meters)
        } else {
          sub.bearing <- subset(sub.heading, bearing.to.target >= low.bearing.1 | bearing.to.target <= high.bearing.1)
          #Restrict Observation range To X * observation distance estimate
          sub.bearing <-subset(sub.bearing,distance.to.target.m <= dist.allowance.factor * obs.dist.estimate.meters)
      ...
      #Order the data by Ascending Track.ID and then by UNIX.Update.Time. Determine Grid location, and if it matches USDA guess. Save result.
        ...
        validate.list$USDA.number.of.birds<-USDA.number.of.birds
        validate.list$USDA.species<-USDA.species
        validate.list$USDA.flight.behavior<-USDA.flight.behavior
        validate.list$USDA.accuracy<-USDA.accuracy
        validate.list$USDA.comments<-USDA.comments
...
#Graph It!!!
      #Create a title and subtitle for the graph based on previous user input
        ...
        oh.title <- paste0("Chicago O'Hare International Airport \n",radar," Tracks ", obs.bearing,"° from Observer with ","Overhead filter from \n ",round(observer.lat,digits=4),"°, ", round(observer.long,digits=4),"° and ",target.heading," Heading ")
      ...
      #Process Results
        ...
        #Convert Data to be plotted into spatial objects
          if(nrow(validate.list) >=1){
          tracks<-SpatialPoints(cbind(validate.list$Longitude,validate.list$Latitude),proj4string=CRS("+proj=longlat"))
          other.tracks<-SpatialPoints(cbind(other.positions$Longitude,other.positions$Latitude),proj4string=CRS("+proj=longlat"))
          radar.pos<-SpatialPoints(cbind(radar.Long,radar.Lat),proj4string=CRS("+proj=longlat"))
          
      #Plot subsetted data, observation point, ARTI position
        par(mar=c(6.1,4.1,5.1,2.1))
        plot(oharemap,xlim=c(-87.997,-87.81),ylim=c(41.938,42.041),axes=TRUE)
        plot(oharegrid,add=TRUE)
          text(coordinates(oharegrid),labels=oharegrid$grid,cex=.5)
        points(other.tracks,col="grey",pch=1)
        points(tracks,col="dark red",pch=1)
        points(radar.pos,col="black",pch=17)
        points(observer.position.longlat,col="blue",pch=10)
        points(observer.bird.position.longlat,col="green",pch=20)
        if(obs.dist.estimate.meters == 0 | obs.dist.estimate.meters <= beam.width.at.bird.guess){title(main=oh.title,sub=sub.title,xlab="Longitude",ylab="Latitude")}
        else{title(main=title,sub=sub.title,xlab="Longitude",ylab="Latitude")}
        legend(-87.855,42.04,c("AR1","Observer","Bird Position Estimate",paste(USDA.number.of.birds,USDA.species)),pch=c(17,10,20,1),col=c("black","blue","green","dark red"),cex=0.60)
      #Add FOV lines to the plot, if not an Overhead observation
        if(obs.dist.estimate.meters == 0 | obs.dist.estimate.meters <= beam.width.at.bird.guess) {
          #Construct a Circle
          radius<-oh.factor * obs.alt.estimate.meters + beam.width.at.bird.guess
          it<-seq(0,2*pi,l=100)
          xt<-obs.UTM.Easting+radius*cos(it)
          yt<-obs.UTM.Northing+radius*sin(it)
          mycircle<-SpatialPoints(cbind(xt,yt),proj4string=CRS("+proj=utm +zone=16T"))
          mycircle.longlat<-spTransform(mycircle, CRS("+proj=longlat"))
          plot(mycircle.longlat,add=T,col="dark cyan",cex=0.25)
          }
        else {
          #Calculate endpoint for lower FOV
            search.distance<-dist.allowance.factor * obs.dist.estimate.meters
            fov.low.bearing<-low.bearing.1 * pi/180
            fov.low.x<-obs.UTM.Easting+search.distance*sin(fov.low.bearing)  ##X UTM Easting
            fov.low.y<-obs.UTM.Northing+search.distance*cos(fov.low.bearing)  ##Y UTM Northing
          #Oh no! We need lat long!
            ...
            low.points<- gcIntermediate(observer.position.longlat,fov.low.position.longlat,n=50,sp=T)
          #Calculate endpoint for upper FOV
            fov.high.bearing<-high.bearing.1 * pi/180
            fov.high.x<-obs.UTM.Easting+search.distance*sin(fov.high.bearing)  ##X UTM Easting
            fov.high.y<-obs.UTM.Northing+search.distance*cos(fov.high.bearing)  ##Y UTM Northing
          #Oh no! We need lat long!
            ...
            high.points<- gcIntermediate(observer.position.longlat,fov.high.position.longlat,n=50,sp=T)
          #Add to plot
      ...
      #Lets Label!
        ...
        text(-88.00,41.94,labels=paste0("'",USDA.comments,"'"),pos=4,cex=.5)
        if(nrow(validate.list)>=1){
      ...
      #assign("unique.track.results.df",unique.track.results.df,.GlobalEnv)
        ...
        if(nrow(validate.list) >= 1){
        dev.copy(png,filename = file.path(working.directory,subDir,subDirPNG,graph.filename), width = 1920, height = 1080 ,res=100)
        dev.off()
        
        #Write a csv file containing the filtered TrackPoints
          write.csv(validate.list[with(validate.list, order(Track.ID, UNIX.Update.Time)), ], file = file.path(working.directory,subDir,subDirCSV,csv.filename),quote=FALSE, row.names = FALSE)
}
        else{
          no.match<-"No tracks matching observation characteristics. Bummer."
          write.csv(no.match, file = file.path(working.directory,subDir,subDirCSV,no.match.csv.filename),quote=FALSE, row.names = FALSE)
        }
      }
else{
  insufficient<-"Insufficient observation information. Ask Sid Majumdar (217-722-3726) why is this is."
  write.csv(insufficient, file = file.path(working.directory,subDir,subDirCSV,insufficient.csv.filename),quote=FALSE, row.names = FALSE)
}

}
else{
  rain<-"Nothing to see here. It was raining. Ask Sid Majumdar (217-722-3726) why it was raining."
  write.csv(rain, file = file.path(working.directory,subDir,subDirCSV,rain.csv.filename),quote=FALSE, row.names = FALSE)
}
}
else{
  no.data<-"No radar data. Nothing to do. Ask Sid Majumdar (217-722-3726) for more radar data."
  write.csv(no.data, file = file.path(working.directory,subDir,subDirCSV,no.data.csv.filename),quote=FALSE, row.names = FALSE) 
}
    }
