...
#Graph It!!!
      ...
      #Plot subsetted data, observation point, ARTI position
        ...
        par(new=T)
        if(obs.dist.estimate.meters == 0 | obs.dist.estimate.meters <= beam.width.at.bird.guess){title(oh.title)}
        else{title(title)}
      ...
      #Add FOV lines to the plot, if not an Overhead observation
        if(obs.dist.estimate.meters == 0 | obs.dist.estimate.meters <= beam.width.at.bird.guess) {
          circle.radius.x<-obs.UTM.Easting+(oh.factor * obs.alt.estimate.meters + beam.width.at.bird.guess)*sin(90)  ##X UTM Easting
          circle.radius.y<-obs.UTM.Northing+(oh.factor * obs.alt.estimate.meters + beam.width.at.bird.guess)*cos(90)  ##Y UTM Northing
          #Oh no! We need lat long!
      ...
      #assign("unique.track.results.df",unique.track.results.df,.GlobalEnv)
        ...
        if(nrow(validate.list) >= 1){
        dev.copy(png,filename = file.path(working.directory,subDir,subDirPNG,graph.filename), width = 1024, height = 905,res=100)
        dev.off()
